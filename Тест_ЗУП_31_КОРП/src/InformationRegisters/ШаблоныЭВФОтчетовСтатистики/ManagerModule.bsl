...
Процедура ЗагрузитьШаблоныЧерезСервисВебСбора(ПараметрыПроцедуры, АдресРезультата, АдресДополнительногоРезультата) Экспорт
	...
	Если СвойстваШаблонов = Неопределено
	 ИЛИ СвойстваШаблонов.Количество() = 0 Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Нет данных для обновления шаблонов.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
		...
		Для НомерШаблона = 1 По КоличествоОбновляемыхШаблонов Цикл
			...
			Исключение
				...
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка доступа к веб-сервису при получении данных шаблона Росстата:
						|%1 ""%2"".'"), СвойстваШаблона.Код, СвойстваШаблона.ИмяФормы);
				Сообщение.Сообщить();
			...
			Если НЕ (ТипЗнч(Ответ) = Тип("ОбъектXDTO")
				...
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Формат ответа веб-сервиса не позволяет получить данные шаблона Росстата:
						|%1 ""%2"".'"), СвойстваШаблона.Код, СвойстваШаблона.ИмяФормы);
				Сообщение.Сообщить();
				КоличествоНезагруженныхШаблонов = КоличествоНезагруженныхШаблонов + 1;
				Продолжить;
			...
			Исключение
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка при распаковке данных шаблона Росстата:
						|%1 ""%2"".'"), СвойстваШаблона.Код, СвойстваШаблона.ИмяФормы);
				Сообщение.Сообщить();
				КоличествоНезагруженныхШаблонов = КоличествоНезагруженныхШаблонов + 1;
				Продолжить;
			...
			Если ДанныеШаблона = Неопределено Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Архив не содержит необходимый файл шаблона:
						|%1 ""%2"".'"), СвойстваШаблона.Код, СвойстваШаблона.ИмяФормы);
				Сообщение.Сообщить();
				КоличествоНезагруженныхШаблонов = КоличествоНезагруженныхШаблонов + 1;
				Продолжить;
	...
	Иначе
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Веб-сервис не поддерживает операцию получения содержимого шаблонов Росстата.'");
		Сообщение.Сообщить();
		Возврат;
	КонецЕсли;
...
Функция СписокШаблоновСервисаВебСбора(СвойстваМодуля, ИспользоватьЦентральныйСервисСбора = Ложь) Экспорт
