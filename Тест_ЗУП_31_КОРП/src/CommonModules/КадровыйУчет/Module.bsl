...
Процедура ОбновитьТекущиеКадровыеДанныеСпискаСотрудников(Сотрудники) Экспорт
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ВЫБОР
		|			КОГДА КадроваяИсторияСотрудниковПрием.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Прием)
		|				ТОГДА КадроваяИсторияСотрудниковПрием.Период
		|			КОГДА КадроваяИсторияСотрудниковПрием.ВидСобытия = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.НачальныеДанные)
		|				ТОГДА ТекущиеКадровыеДанныеСотрудников.ДатаПриема
		|			ИНАЧЕ ДАТАВРЕМЯ(1, 1, 1)
		|		КОНЕЦ, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаПриема,
