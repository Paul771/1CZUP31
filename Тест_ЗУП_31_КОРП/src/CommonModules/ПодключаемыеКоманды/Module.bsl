...
Процедура ВывестиКоманды(Форма, Команды, ПараметрыРазмещения)
	...
	Для Каждого ВидКоманд Из ВидыКоманд Цикл
		...
		ВыводБезПодменю = (ВидКоманд.Имя = "КоманднаяПанель");
		...
		Для Каждого Команда Из КомандыВида Цикл
			...
			ИмяКорневогоЭлемента = ?(ВыводБезПодменю, Команда.ИмяВФорме, СведенияОПодменюКоманды.Подменю.Имя);
			...
			КонецЕсли;
			Если ВыводБезПодменю И Команда.Важность = "СмТакже" Тогда
