...
Процедура ПКО_Документ_НачислениеОценочныхОбязательствПоОтпускам_ПриОтправкеДанных(ДанныеИБ, ДанныеXDTO, КомпонентыОбмена, СтекВыгрузки)
	...
	КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(
		Ложь,
		ДанныеИБ.РасчетРезерваОтпусков.ВыгрузитьКолонку("Сотрудник"),
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ВТКадровыеДанные.Подразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)) КАК ПодразделениеРегл,
	|	ЕСТЬNULL(ВТКадровыеДанные.МестоВСтруктуреПредприятия, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)) КАК ПодразделениеПредприятия,
	|	СУММА(РасчетРезерваОтпусков.СуммаРезерва) КАК СуммаРезерва,
	|	СУММА(РасчетРезерваОтпусков.СуммаРезерваНУ) КАК СуммаРезерваНУ,
	|	СУММА(РасчетРезерваОтпусков.СуммаРезерваСтраховыхВзносов) КАК СуммаРезерваСтраховыхВзносов,
	|	СУММА(РасчетРезерваОтпусков.СуммаРезерваСтраховыхВзносовНУ) КАК СуммаРезерваСтраховыхВзносовНУ,
	|	СУММА(РасчетРезерваОтпусков.СуммаРезерваФССНесчастныеСлучаи) КАК СуммаРезерваФССНесчастныеСлучаи,
	|	СУММА(РасчетРезерваОтпусков.СуммаРезерваФССНесчастныеСлучаиНУ) КАК СуммаРезерваФССНесчастныеСлучаиНУ
	|ИЗ
	...
	|		ПО (ВТКадровыеДанные.Сотрудник = РасчетРезерваОтпусков.Сотрудник)
	|ГДЕ
	|	РасчетРезерваОтпусков.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(ВТКадровыеДанные.МестоВСтруктуреПредприятия, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)),
	|	ЕСТЬNULL(ВТКадровыеДанные.Подразделение, ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка))
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СтруктураПредприятия.Источник КАК ПодразделениеРегл,
	|	МАКСИМУМ(СтруктураПредприятия.Ссылка) КАК ПодразделениеУпр
	|ПОМЕСТИТЬ ВТСоответствияПодразделений
	|ИЗ
	|	Справочник.СтруктураПредприятия КАК СтруктураПредприятия
	|СГРУППИРОВАТЬ ПО
	|	СтруктураПредприятия.Источник
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОценочныеОбязательства.ОблагаетсяЕНВД КАК ОблагаетсяЕНВД,
	|	ОценочныеОбязательства.Подразделение КАК ПодразделениеРегл,
	|	ЕСТЬNULL(ВТСоответствияПодразделений.ПодразделениеУпр, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)) КАК ПодразделениеПредприятия,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСоответствияПодразделений КАК ВТСоответствияПодразделений
	|		ПО ВТСоответствияПодразделений.ПодразделениеРегл = ОценочныеОбязательства.Подразделение
	|ГДЕ
	...
	РасчетРезерваОтпусковУпр   = РезультатыЗапроса[1].Выгрузить();
	ОценочныеОбязательстваРегл = РезультатыЗапроса[3].Выгрузить();
	
	Если ДанныеИБ.КорректировкаОбязательств Тогда
		ОценочныеОбязательстваУпр = ОценочныеОбязательстваРегл.Скопировать();
	Иначе
		ОценочныеОбязательстваУпр = ОценочныеОбязательстваРегл.СкопироватьКолонки();
		
		СуммыРаспределено = Новый Структура;
		СуммыРаспределено.Вставить("СуммаРезерва",						0);
		СуммыРаспределено.Вставить("СуммаРезерваНУ",					0);
		СуммыРаспределено.Вставить("СуммаРезерваСтраховыхВзносов",		0);
		СуммыРаспределено.Вставить("СуммаРезерваСтраховыхВзносовНУ",	0);
		СуммыРаспределено.Вставить("СуммаРезерваФССНесчастныеСлучаи",	0);
		СуммыРаспределено.Вставить("СуммаРезерваФССНесчастныеСлучаиНУ",	0);
	
		Для Каждого СтрокаРасчетРезерваОтпусковУпр Из РасчетРезерваОтпусковУпр Цикл
			ОценочныеОбязательстваРеглПоПодразделению = ОценочныеОбязательстваРегл.Скопировать(Новый Структура("ПодразделениеРегл",
				СтрокаРасчетРезерваОтпусковУпр.ПодразделениеРегл));
				
			СтрокаОценочныеОбязательстваУпр = Неопределено;
			
			Для Каждого СуммаРаспределено Из СуммыРаспределено Цикл
				СуммыРаспределено[СуммаРаспределено.Ключ] = 0;
			КонецЦикла;
			
			Для Каждого СтрокаОценочныеОбязательстваРеглПоПодразделению Из ОценочныеОбязательстваРеглПоПодразделению Цикл
				СтрокаОценочныеОбязательстваУпр = ОценочныеОбязательстваУпр.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаОценочныеОбязательстваУпр, СтрокаОценочныеОбязательстваРеглПоПодразделению,
					"ОблагаетсяЕНВД, СпособОтраженияЗарплатыВФинансовомУчете, СтатьяФинансирования");
					
				СтрокаОценочныеОбязательстваУпр.ПодразделениеПредприятия = СтрокаРасчетРезерваОтпусковУпр.ПодразделениеПредприятия;
				
				Для Каждого СуммаРаспределено Из СуммыРаспределено Цикл
					Если ОценочныеОбязательстваРеглПоПодразделению.Итог(СуммаРаспределено.Ключ) = 0 Тогда
						Продолжить;
					КонецЕсли;
					
					ТекущаяСумма = Окр(СтрокаРасчетРезерваОтпусковУпр[СуммаРаспределено.Ключ] * СтрокаОценочныеОбязательстваРеглПоПодразделению[СуммаРаспределено.Ключ] / ОценочныеОбязательстваРеглПоПодразделению.Итог(СуммаРаспределено.Ключ), 2);
					
					СтрокаОценочныеОбязательстваУпр[СуммаРаспределено.Ключ] = ТекущаяСумма;				
					СуммыРаспределено[СуммаРаспределено.Ключ] = СуммыРаспределено[СуммаРаспределено.Ключ] + ТекущаяСумма;
				КонецЦикла;			
			КонецЦикла;
			
			Если СтрокаОценочныеОбязательстваУпр = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			Для Каждого СуммаРаспределено Из СуммыРаспределено Цикл
				СуммаДельта = СуммаРаспределено.Значение - ОценочныеОбязательстваРеглПоПодразделению.Итог(СуммаРаспределено.Ключ);
				
				Если СуммаДельта = 0 Тогда
					Продолжить;
				КонецЕсли;
				
				СтрокаОценочныеОбязательстваРеглПоПодразделению[СуммаРаспределено.Ключ] = СтрокаОценочныеОбязательстваРеглПоПодразделению[СуммаРаспределено.Ключ] - СуммаДельта;
			КонецЦикла;
		КонецЦикла;
	КонецЕсли;
	
	ОценочныеОбязательстваУпр.Колонки.Добавить("Подразделение");
	ЗаполнитьПодразделениеПоУмолчанию(ОценочныеОбязательстваУпр);
	ДанныеXDTO.Вставить("ОценочныеОбязательства", ОценочныеОбязательстваУпр);
КонецПроцедуры
#КонецОбласти
...
Процедура ПКО_Документ_ОтражениеЗарплатыВБухучете_ПриОтправкеДанных(ДанныеИБ, ДанныеXDTO, КомпонентыОбмена, СтекВыгрузки)
	...
	СписокФизЛиц = ДанныеИБ.НачисленнаяЗарплатаИВзносы.ВыгрузитьКолонку("ФизическоеЛицо");
	...
	|	КадровыеДанные.МестоВСтруктуреПредприятия КАК МестоВСтруктуреПредприятия
	|ПОМЕСТИТЬ ВТКадровыеДанные
	|ИЗ
	|	&КадровыеДанные КАК КадровыеДанные
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НачисленнаяЗарплатаИВзносы.Подразделение КАК ПодразделениеРегл,
	|	МИНИМУМ(ЕСТЬNULL(СтруктураПредприятия.Ссылка, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка))) КАК ПодразделениеУпр
	|ПОМЕСТИТЬ ВТСоответствияПодразделенийНачисления
	|ИЗ
	|	Документ.ОтражениеЗарплатыВБухучете.НачисленнаяЗарплатаИВзносы КАК НачисленнаяЗарплатаИВзносы
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтруктураПредприятия КАК СтруктураПредприятия
	|		ПО (СтруктураПредприятия.Источник = НачисленнаяЗарплатаИВзносы.Подразделение)
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	УдержаннаяЗарплата.Подразделение КАК ПодразделениеРегл,
	|	МИНИМУМ(ЕСТЬNULL(СтруктураПредприятия.Ссылка, ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка))) КАК ПодразделениеУпр
	|ПОМЕСТИТЬ ВТСоответствияПодразделенийУдержания
	|ИЗ
	|	Документ.ОтражениеЗарплатыВБухучете.УдержаннаяЗарплата КАК УдержаннаяЗарплата
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.СтруктураПредприятия КАК СтруктураПредприятия
	|		ПО (СтруктураПредприятия.Источник = УдержаннаяЗарплата.Подразделение)
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(ВТКадровыеДанные.МестоВСтруктуреПредприятия, ВТСоответствияПодразделенийНачисления.ПодразделениеУпр) КАК ПодразделениеПредприятия,
	...
	|	Документ.ОтражениеЗарплатыВБухучете.НачисленнаяЗарплатаИВзносы КАК НачисленнаяЗарплатаИВзносы
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКадровыеДанные КАК ВТКадровыеДанные
	|		ПО (ВТКадровыеДанные.ФизическоеЛицо = НачисленнаяЗарплатаИВзносы.ФизическоеЛицо)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСоответствияПодразделенийНачисления КАК ВТСоответствияПодразделенийНачисления
	|		ПО (ВТСоответствияПодразделенийНачисления.ПодразделениеРегл = НачисленнаяЗарплатаИВзносы.Подразделение)
	|ГДЕ
	|	НачисленнаяЗарплатаИВзносы.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	ЕСТЬNULL(ВТКадровыеДанные.МестоВСтруктуреПредприятия, ВТСоответствияПодразделенийНачисления.ПодразделениеУпр),
	...
	|	ЕСТЬNULL(ВТКадровыеДанные.МестоВСтруктуреПредприятия, ВТСоответствияПодразделенийУдержания.ПодразделениеУпр) КАК ПодразделениеПредприятия,
	...
	|	Документ.ОтражениеЗарплатыВБухучете.УдержаннаяЗарплата КАК УдержаннаяЗарплата
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКадровыеДанные КАК ВТКадровыеДанные
	|		ПО (ВТКадровыеДанные.ФизическоеЛицо = УдержаннаяЗарплата.ФизическоеЛицо)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСоответствияПодразделенийУдержания КАК ВТСоответствияПодразделенийУдержания
	|		ПО (ВТСоответствияПодразделенийУдержания.ПодразделениеРегл = УдержаннаяЗарплата.Подразделение)
	|ГДЕ
	|	УдержаннаяЗарплата.Ссылка = &Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	УдержаннаяЗарплата.ФизическоеЛицо,
	|	ЕСТЬNULL(ВТКадровыеДанные.МестоВСтруктуреПредприятия, ВТСоответствияПодразделенийУдержания.ПодразделениеУпр),
	...
	ТаблицаНачисленнаяЗарплатаИВзносы = РезультатыЗапроса[3].Выгрузить();
	...
	// НачисленныйНДФЛ
	ТаблицаНачисленныйНДФЛ = РезультатыЗапроса[4].Выгрузить();
	...
	// УдержаннаяЗарплата
	ТаблицаУдержаннаяЗарплата = РезультатыЗапроса[5].Выгрузить();
