...
Процедура ОповеститьОЗавершенииБизнесПроцесса(ОбработчикиЗавершенияБизнесПроцесса, КонтекстВзаимодействия)
	...
	Если КонтекстВзаимодействия <> Неопределено
		И КонтекстВзаимодействия.Свойство("ОбработчикиСобытийБизнесПроцесса")
		И КонтекстВзаимодействия.ОбработчикиСобытийБизнесПроцесса <> Неопределено Тогда
		КонтекстВзаимодействия.ОбработчикиСобытийБизнесПроцесса = Неопределено;
	КонецЕсли;
	...
	Если ОбработчикиЗавершенияБизнесПроцесса.Обработчик <> Неопределено Тогда
		...
		ОбработчикиЗавершенияБизнесПроцесса.Обработчик = Неопределено;
	КонецЕсли;
