...
Функция ОшибкаНеУстановленКриптопровайдер(Криптопровайдер) Экспорт
	...
	Если ЗначениеЗаполнено(Криптопровайдер) Тогда
		Если КриптографияЭДКОКлиентСервер.ЭтоКриптопровайдерCryptoPro(Криптопровайдер) Тогда
			Возврат	КриптографияЭДКОСлужебныйКлиентСервер.ОшибкаНеУстановленКриптопровайдерCryptoPro();
		ИначеЕсли КриптографияЭДКОКлиентСервер.ЭтоКриптопровайдерViPNet(Криптопровайдер) Тогда
...
Функция ОшибкаНеУстановленНиОдинИзПоддерживаемыхКриптопровайдеров() Экспорт
	...
	Для Каждого ПоддерживаемыйКриптопровайдер Из КриптографияЭДКОКлиентСервер.ПоддерживаемыеКриптопровайдеры() Цикл
