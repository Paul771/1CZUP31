...
Функция ПолучитьТекстОбщегоЗапроса(ИсточникДанных) Экспорт
	...
	Если ВРег(ИсточникДанных) = ВРег("НачисленияУдержания") Тогда
		...
		|ГДЕ
		|	РАЗНОСТЬДАТ(ОстаткиИОбороты.Период, СотрудникиМесяцы.Месяц, МЕСЯЦ) <= 1
		|	И (ОстаткиИОбороты.КонечныйОстаток <> 0
		|			ИЛИ ОстаткиИОбороты.НачальныйОстаток <> 0)
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ЕСТЬNULL(ВЫБОР
		|			КОГДА СотрудникиМесяцыОстатков.Месяц = ОстаткиИОбороты.Период
		|				ТОГДА ОстаткиИОбороты.КонечныйОстаток
		|			ИНАЧЕ ОстаткиИОбороты.НачальныйОстаток
		...
		|		ПО (ПоследниеПервыеМесяцыСальдо.Организация = ВзаиморасчетыССотрудниками.Организация)
		|			И (ПоследниеПервыеМесяцыСальдо.Сотрудник = ВзаиморасчетыССотрудниками.Сотрудник)
		|			И (ПоследниеПервыеМесяцыСальдо.ФизическоеЛицо = ВзаиморасчетыССотрудниками.ФизическоеЛицо)
		|			И (ПоследниеПервыеМесяцыСальдо.Подразделение = ВзаиморасчетыССотрудниками.Подразделение)
		|			И (ПоследниеПервыеМесяцыСальдо.СтатьяФинансирования = ВзаиморасчетыССотрудниками.СтатьяФинансирования)
		|			И (ПоследниеПервыеМесяцыСальдо.СтатьяРасходов = ВзаиморасчетыССотрудниками.СтатьяРасходов)
		...
		|ГДЕ
		|	РАЗНОСТЬДАТ(БухгалтерскиеОстаткиИОбороты.Период, СотрудникиМесяцы.Месяц, МЕСЯЦ) <= 1
		|	И (БухгалтерскиеОстаткиИОбороты.КонечныйОстаток <> 0
		|			ИЛИ БухгалтерскиеОстаткиИОбороты.НачальныйОстаток <> 0)
		|
		|СГРУППИРОВАТЬ ПО
		|	СотрудникиМесяцы.Организация,
		|	СотрудникиМесяцы.Сотрудник,
		...
		|	ЕСТЬNULL(ВЫБОР
		|			КОГДА СотрудникиМесяцыБухгалтерскихОстатков.Месяц = БухгалтерскиеОстаткиИОбороты.Период
		|				ТОГДА БухгалтерскиеОстаткиИОбороты.НачальныйОстаток
		|			ИНАЧЕ БухгалтерскиеОстаткиИОбороты.КонечныйОстаток
		|		КОНЕЦ, 0) КАК НачальныйОстаток,
		|	ЕСТЬNULL(ВЫБОР
		|			КОГДА СотрудникиМесяцыБухгалтерскихОстатков.Месяц = БухгалтерскиеОстаткиИОбороты.Период
		|				ТОГДА БухгалтерскиеОстаткиИОбороты.КонечныйОстаток
		|			ИНАЧЕ БухгалтерскиеОстаткиИОбороты.НачальныйОстаток
		|		КОНЕЦ, 0) КАК КонечныйОстаток,
		...
		|			И СотрудникиМесяцыБухгалтерскихОстатков.ПоследнийМесяцОстатков = БухгалтерскиеОстаткиИОбороты.Период
		|ГДЕ
		|	(НЕ БухгалтерскиеОстаткиИОбороты.НачальныйОстаток ЕСТЬ NULL
		...
		|		ПО (ПоследниеПервыеМесяцыСальдо.Организация = БухгалтерскиеВзаиморасчетыССотрудниками.Организация)
		|			И (ПоследниеПервыеМесяцыСальдо.Сотрудник = БухгалтерскиеВзаиморасчетыССотрудниками.Сотрудник)
		|			И (ПоследниеПервыеМесяцыСальдо.ФизическоеЛицо = БухгалтерскиеВзаиморасчетыССотрудниками.ФизическоеЛицо)
		|			И (ПоследниеПервыеМесяцыСальдо.Подразделение = БухгалтерскиеВзаиморасчетыССотрудниками.Подразделение)
		|			И (ПоследниеПервыеМесяцыСальдо.СтатьяФинансирования = БухгалтерскиеВзаиморасчетыССотрудниками.СтатьяФинансирования)
		|			И (ПоследниеПервыеМесяцыСальдо.СтатьяРасходов = БухгалтерскиеВзаиморасчетыССотрудниками.СтатьяРасходов)
		|		{ЛЕВОЕ СОЕДИНЕНИЕ Представления_КадровыеДанныеСотрудниковАнализНачисленийИУдержаний КАК КадровыеДанныеГоловныхСотрудников
...
Функция ПолучитьЗапросПоПредставлению(ТекстЗапроса, СоответствиеПараметров) Экспорт
	...
	ИначеЕсли ВРег(ИмяПредставления) = ВРег("КадровыеДанныеСотрудниковАнализНачисленийИУдержаний") Тогда
		Запрос = ЗарплатаКадрыОбщиеНаборыДанных.ЗапросПредставленияКадровыеДанныеСотрудниковАнализНачисленийИУдержаний(ТекстЗапроса);
...
Функция ЗапросПредставленияКадровыеДанныеСотрудниковАнализНачисленийИУдержаний(ТекстИсходногоЗапроса) Экспорт
	...
	Запрос.Текст =
		...
		|		ПО (ЕСТЬNULL(КадровыеДанныеСотрудников.Организация, Организации.Ссылка) = Организации.Ссылка)";
