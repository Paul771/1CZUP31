...
Функция СписокВыбораТипов()
	...
	МассивТипов = Метаданные.ОпределяемыеТипы.ИсточникДокументаПоТребованиюФНСБРО.Тип.Типы();
	
	ТаблицаТипов = Новый ТаблицаЗначений;
	ТаблицаТипов.Колонки.Добавить("Тип");
	ТаблицаТипов.Колонки.Добавить("НаименованиеТипа");
	
	Для каждого ТипДокумента Из МассивТипов Цикл
		
		НоваяСтрока = ТаблицаТипов.Добавить();
		НоваяСтрока.Тип = ТипДокумента;
		НоваяСтрока.НаименованиеТипа = Строка(ТипДокумента);
		
	КонецЦикла;
	
	ТаблицаТипов.Сортировать("НаименованиеТипа");
	
	СписокЗначений = Новый СписокЗначений;
	Для каждого СтрокаТаблицыТипов Из ТаблицаТипов Цикл
		СписокЗначений.Добавить(СтрокаТаблицыТипов.Тип);
	КонецЦикла;
...
&НаКлиенте
Процедура ВыборИсточникаПослеВыбораТипа(Результат, ВходящийКонтекст) Экспорт
	...
	Если СтандартнаяОбработка = Истина Тогда
		// Случай, когда переопределяемая не заполнена
		
		ВыбратьДокументИсточникДляПередачиФНС(ВыбранныйТип, ВыполняемоеОповещение);
...
&НаКлиенте
Процедура ВыбратьДокументИсточникДляПередачиФНС(Тип, ВыполняемоеОповещение) Экспорт

	СтандартнаяОбработка = Ложь;
	
	ИмяФормыИсточника = ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ПолучитьИмяФормыОбъктаДляПередачиФНС(Тип);
	
	Если ИмяФормыИсточника = Неопределено Тогда
	...
	ДополнительныеПараметры.Вставить("Тип", Тип);
	...
	ОткрытьФорму(ИмяФормыИсточника,, ЭтотОбъект, , , , ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);

КонецПроцедуры

&НаКлиенте
Процедура ВыбратьДокументИсточникДляПередачиФНСЗавершение(Результат, ВходящийКонтекст) Экспорт
	
	Если ЗначениеЗаполнено(Результат)
		И ТипЗнч(Результат) = ВходящийКонтекст.Тип Тогда
	
		Свойства = Новый Структура;
		Свойства.Вставить("Ссылка", 	Результат);
		Свойства.Вставить("Описание", 	Строка(Результат));
		
		ВыполнитьОбработкуОповещения(ВходящийКонтекст.ВыполняемоеОповещение, Свойства);
...
&НаКлиенте
Процедура ВыборИсточникаПослеПолученияСвойствДокумента(СвойстваВыбранногоДокумента, ВходящийКонтекст) Экспорт
	
	Если ТипЗнч(СвойстваВыбранногоДокумента) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	
	Модифицированность = Истина;
	
	Если СвойстваВыбранногоДокумента.Свойство("Ссылка") Тогда
		Объект.Источник = СвойстваВыбранногоДокумента.Ссылка;
	КонецЕсли;
	
	Если СвойстваВыбранногоДокумента.Свойство("Описание") Тогда
		Объект.СведенияДокумента = СвойстваВыбранногоДокумента.Описание;
	КонецЕсли;
	
	Если СвойстваВыбранногоДокумента.Свойство("Основание") Тогда
		Объект.СведенияДокументаОснования = СвойстваВыбранногоДокумента.Основание;
	КонецЕсли;
...
Процедура ВыбратьИсточник(Элемент, СтандартнаяОбработка)
	...
	ВыбираемыеТипов = СписокВыбораТипов();
	
	КоличествоТипов = ВыбираемыеТипов.Количество();
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ВыборИсточникаПослеВыбораТипа", ЭтотОбъект);
	
	Если КоличествоТипов = 1 Тогда
		// пропускаем выбор типа
		ВыполнитьОбработкуОповещения(ОписаниеОповещения, ВыбираемыеТипов[0]);
	Иначе
		ТекущийТип = ТипЗнч(Объект.Источник);
		ВыбираемыеТипов.ПоказатьВыборЭлемента(ОписаниеОповещения, , ТекущийТип);
	КонецЕсли;
