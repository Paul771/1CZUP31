...
Функция ЗагруженныйДокументСЗВ_6(ДеревоСведенийСЗВ, ПачкиСоответствующиеФайлам, МассивНеСоответствующихФайламПачек, Классификаторы)
	...
	Для каждого СтрокаЗастрахованного Из МассивЗастрахованных Цикл
		...
		Если НовоеФизЛицо И ЗначениеЗаполнено(НоваяСтрока.АдресДляИнформирования) Тогда
			...
			НоваяСтрока.Вид = Справочники.ВидыКонтактнойИнформации.АдресДляИнформированияФизическиеЛица;
			НоваяСтрока.ЗначенияПолей = НоваяСтрока.АдресДляИнформирования;
			НоваяСтрока.Представление = НоваяСтрока.АдресДляИнформированияПредставление;
			
			// Для нового ФизЛица сохраним адрес для информирования.
			УправлениеКонтактнойИнформацией.УстановитьКонтактнуюИнформациюОбъекта(ФизическоеЛицо, КонтактнаяИнформация);
