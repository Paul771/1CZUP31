...
Процедура ОбработатьНаборЗаписейСОтборомСубъекта(НаборЗаписейРегистра, ОбъектМетаданных, ПараметрыОтбора)
	...
	Субъекты = ПараметрыОтбора.Субъекты;
	ТипСубъектов = ПараметрыОтбора.ТипСубъектов;
	ИменаИзмеренийСТипомСубъекта = ИменаИзмеренийРегистраПоТипу(МетаданныеРегистра, ТипСубъектов);
...
Процедура ОбработатьОбъектМетаданных(ОбъектМетаданных, ПараметрыОтбора)
	...
	Субъекты = ПараметрыОтбора.Субъекты;
	ТипСубъектов = ПараметрыОтбора.ТипСубъектов;
	ИменаРеквизитовСТипомСубъекта = "";
	РеквизитыПроверены = Ложь;
	...
	Иначе
		...
		Результат = ДанныеОбъектаМетаданных(ОбъектМетаданных, Ссылка);
	...
	Пока Истина Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			Если ЗначениеЗаполнено(Субъекты) И Не РеквизитыПроверены Тогда
				ИменаРеквизитовСТипомСубъекта = ОбщегоНазначения.ИменаРеквизитовПоТипу(СтрокаРезультата.Ссылка, ТипСубъектов);
				РеквизитыПроверены = Истина;
			КонецЕсли;
			...
			Для Каждого ГруппаСвойствОбъекта Из ОбъектМетаданных.Строки Цикл
				...
				Для Каждого ОбрабатываемоеСвойство Из ГруппаСвойствОбъекта.Строки Цикл
					...
					Если ГруппаСвойствОбъекта.Тип = "ТабличнаяЧасть" Тогда
						...
						Если ЗначениеЗаполнено(Субъекты) И ПустаяСтрока(ИменаРеквизитовСТипомСубъекта) Тогда
							
							ИменаРеквизитовТЧСТипомСубъекта = ИменаРеквизитовТЧОбъектаПоТипу(ОбрабатываемыйОбъект.Метаданные(), ГруппаСвойствОбъекта.Имя, ТипСубъектов);
