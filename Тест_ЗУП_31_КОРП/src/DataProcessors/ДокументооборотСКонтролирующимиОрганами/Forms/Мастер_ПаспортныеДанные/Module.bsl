...
Процедура ИзменитьОформлениеКраснойПанелиИСкорректироватьВидДокумента()
	...
	Если ВыбранНеПаспорт И ЗначениеЗаполнено(ВладелецЭЦП) Тогда
		...
		ТекстПредупрежденияНачало = НСтр("ru = 'Реквизиты удостоверения не могут быть заполнены по данным карточки физ.лица, поскольку в качестве документ разрешено указывать только российский или иностранный паспорт. Заполните сведения в карточке физ. лица '");
