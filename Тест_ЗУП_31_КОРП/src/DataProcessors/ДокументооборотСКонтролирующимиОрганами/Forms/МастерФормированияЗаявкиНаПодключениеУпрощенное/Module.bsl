...
Процедура ОпределитьВозможностьБезбумажногоПодписания_ПослеОпределенияКриптопровайдера(Результат, ВходящийКонтекст) Экспорт
	...
	Если ВсеСертификаты = Неопределено Тогда
		...
		ХранилищеСертификатов = Новый Структура("Хранилище, ЭтоЛокальноеХранилище", "MY", Истина);
		КриптографияЭДКОКлиент.ПолучитьСертификаты(ОписаниеОповещения, ХранилищеСертификатов, ДополнительныеПараметрыМетода);
