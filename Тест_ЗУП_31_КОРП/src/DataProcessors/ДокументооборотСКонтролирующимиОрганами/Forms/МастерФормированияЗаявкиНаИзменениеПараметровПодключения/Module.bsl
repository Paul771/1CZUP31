...
Процедура ПодписатьЗаявлениеИОтправить(Команда)
	...
	Если ВладелецЭЦПИзменилисьПаспортныеДанные И НЕ ЗначениеЗаполнено(ФайлСканПаспорт) Тогда
...
Функция БезбумажноеПродлениеВозможно(Форма)
	...
	Возврат 
		...
		И Форма.ВладелецЭЦПТип = ПредопределенноеЗначение("Перечисление.ТипыВладельцевЭЦП.Руководитель") И Форма.ПаспортныеДанныеИзменены;
...
Процедура ПроверитьКорректностьМобильногоИПочты(МастерДалее)
	...
	Если ЭтоУчетнаяЗаписьВМоделиСервиса Тогда
		...
		Если Не ТелефонДляПаролейИзменен И Не ЭлектроннаяПочтаДляПаролейИзменена И НЕ ПолучатьУведомленияИзменен Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Укажите новый номер мобильного телефона'"),, "ТелефонДляПаролей");
			МастерДалее = Ложь;
...
Процедура УстановитьЗаголовокКлючБудетСозданнойПрограммой()
	...
	Иначе
		...
		Если CryptoProCSPУстановлен И НЕ ИгнорироватьКонфликт 
			ИЛИ ИгнорироватьКонфликт И ВыбранноеМестоХраненияКлюча = ПредопределенноеЗначение("Перечисление.ТипыКриптоПровайдеров.CryptoPro") Тогда
			СвойстваКриптопровайдера = КриптографияЭДКОКлиентСервер.КриптопровайдерCryptoPro();
			Представление = СвойстваКриптопровайдера.Представление;
		Иначе
			СвойстваКриптопровайдера = КриптографияЭДКОКлиентСервер.КриптопровайдерViPNet();
			Представление = СвойстваКриптопровайдера.Представление;
		КонецЕсли;
...
Процедура УстановитьГалкуИзменитьВладельцаСертификата()
	...
	Иначе
		...
		Если (ПодразделениеОтличается ИЛИ ДолжностьОтличается ИЛИ ВладелецЭЦПИзменилисьПаспортныеДанные) И ЗначениеЗаполнено(ВладелецЭЦП) Тогда
...
Процедура ДобавитьВТаблицуКриптопровайдерИМодельХраненияКлючей()
	...
	Если ЭтоПереходВКоробку ИЛИ ЭтоПереходВОблако ИЛИ ТипКриптопровайдераИзменился Тогда
		...
		Иначе
			СвойстваКриптопровайдера 		= ЭлектронныйДокументооборотСКонтролирующимиОрганамиКлиентСервер.ПолучитьПараметрыКриптопровайдера(ВыбранноеМестоХраненияКлюча);
			ПредставлениеКриптопровайдера 	= СвойстваКриптопровайдера.ОтображаемоеНазвание; 
		КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ТелефонМобильный.Видимость = Не Форма.ЭтоУчетнаяЗаписьВМоделиСервиса;
	Элементы.ТелефонДляПаролей.Видимость = Форма.ЭтоУчетнаяЗаписьВМоделиСервиса;
	
	// Телефон для паролей
	Элементы.КартинкаТелефонПроверен.Видимость = 
		Форма.ПроверкаТелефонДляПаролей.ПодтверждениеВыполнено И Форма.ЭтоУчетнаяЗаписьВМоделиСервиса;
	Элементы.ПроверитьНомер.Видимость = 
		...
		И Форма.ЭтоУчетнаяЗаписьВМоделиСервиса;
	...
	Элементы.ОтправитьКодПовторноТелефон.Видимость = 
		Не Форма.ПроверкаТелефонДляПаролей.КодОтправлен И Форма.ЭтоУчетнаяЗаписьВМоделиСервиса;
	Элементы.НадписьОбратногоОтсчетаТелефон.Видимость = 
		Форма.ПроверкаТелефонДляПаролей.КодОтправлен И Форма.ЭтоУчетнаяЗаписьВМоделиСервиса;
	
	
	// Электронная почта для паролей
	Элементы.КартинкаЭлектроннаяПочтаПроверена.Видимость =
		Форма.ПроверкаЭлектроннаяПочтаДляПаролей.ПодтверждениеВыполнено И Форма.ЭтоУчетнаяЗаписьВМоделиСервиса;
	...
	Элементы.ОтправитьКодПовторноЭлектроннаяПочта.Видимость = 
		Не Форма.ПроверкаЭлектроннаяПочтаДляПаролей.КодОтправлен И Форма.ЭтоУчетнаяЗаписьВМоделиСервиса;
	Элементы.НадписьОбратногоОтсчетаЭлектроннаяПочта.Видимость = 
		Форма.ПроверкаЭлектроннаяПочтаДляПаролей.КодОтправлен И Форма.ЭтоУчетнаяЗаписьВМоделиСервиса;
	...
	Элементы.ОтступМеждуТелефономИЭлектроннойПочтой.Видимость = Форма.ЭтоУчетнаяЗаписьВМоделиСервиса;
	Элементы.ГруппаЭлектроннаяПочта.Видимость = Форма.ЭтоУчетнаяЗаписьВМоделиСервиса;
	...
	Элементы.СканПаспорт.Видимость = БезбумажноеПродлениеВозможно(Форма) И Форма.ВладелецЭЦПИзменилисьПаспортныеДанные;
