...
Функция ПроверитьВозможностьВыгрузки(ИмяФормы,
	...
	УказанГлавныйБухгалтер = ЗначениеЗаполнено(ПараметрыВыгрузки.ФамилияОтв)
						 ИЛИ ЗначениеЗаполнено(ПараметрыВыгрузки.ИмяОтв)
						 ИЛИ ЗначениеЗаполнено(ПараметрыВыгрузки.ОтчествоОтв);
						 
	Если УказанГлавныйБухгалтер Тогда
		Если ПустаяСтрока(ПараметрыВыгрузки.ФамилияОтв) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
				НСтр("ru='Не удалось определить фамилию ответственного сотрудника (главного бухгалтера).'"),
				"Титульный", "");
			СтатусПроверки = Ложь;
		КонецЕсли;
		...
		Если ПустаяСтрока(ПараметрыВыгрузки.ИмяОтв) Тогда
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(РезультатВыгрузки,
				НСтр("ru='Не удалось определить имя ответственного сотрудника (главного бухгалтера).'"),
				"Титульный", "");
			СтатусПроверки = Ложь;
		КонецЕсли;
	КонецЕсли;
